...
describe(`TypeOrmUserRepository - e2e test`, () => {
  ...
  describe(`getOne`, () => {
    ...
    test(`When user exists, finding either by id or email will be successfull.`, async () => {
      const user = await User.new({id: id, email: email, role: role, password: password});
      ...
      ExpectUtils.data({data: user1, passFields: ['id', 'email', 'role', 'password']}, {
        id: user.getId(),
        email: user.getEmail(),
        role: user.getRole(),
    ...
    test(`When user exists, finding user with invalid pair (email, id) should return undefined`, async () => {
      const user = await userRepo.getOne({id: id, email: 'admin@miracleforyou.com'});
